...
class AsyncQueueSpec extends AnyFreeSpec with ChiselScalatestTester {

  "AsyncQueue test" in {
    test(new AsyncQueue()) { dut =>
      ...
      // val inputSeq = testValues.map { case (x, y) => (new GcdInputBundle(params(width))).Lit(_.value1 -> x.U, _.value2 -> y.U) }
      // val resultSeq = testValues.map { case (x, y) =>
      //   (new GcdOutputBundle(params(width))).Lit(_.value1 -> x.U, _.value2 -> y.U, _.gcd -> BigInt(x).gcd(BigInt(y)).U)
      // }
